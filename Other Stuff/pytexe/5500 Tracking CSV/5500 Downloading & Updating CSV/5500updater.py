...
for year in yearList:
	...
	driver = webdriver.Chrome('\\\\Mowgli\\Research\\Fund Performance\\5500 & 990\\5500\\5500 Tracking CSV\\5500 Downloading & Updating CSV\\Chrome Selenium Web Driver\\chromechromedriver.exe')  #arg is the path where the chrome driver is located
	driver.implicitly_wait(10) #Sets the wait time for the browser checks every .5 seconds
	driver.get("http://askebsa.dol.gov/FOIA%20Files/" + str(year) + "/Latest/F_SCH_H_" + str(year) + "_Latest.zip")
	while os.path.exists("C:\\Users\\"+ userName +"\\Downloads\\F_SCH_H_"+ str(year) +"_Latest.zip") is False:
				time.sleep(2)	
	driver.quit()
	os.rename("C:\\Users\\"+ userName +"\\Downloads\\F_SCH_H_"+ str(year) +"_Latest.zip","\\Downloads\\F_SCH_H_"+ str(year) +"_Latest.zip","\\\\Mowgli\\Research\\Fund Performance\\5500 & 990\\5500\\5500 Tracking CSV\\5500 Downloading & Updating CSV\\Updater\\F_SCH_H_"+ str(year) +"_Latest.zip")
...
try:
	...
	for files in os.listdir("\\\\Mowgli\\Research\\Fund Performance\\5500 & 990\\5500\\5500 Tracking CSV\\5500 Downloading & Updating CSV\\Updater"):
		if files.endswith(".csv") or files.endswith(".zip") or files.endswith(".txt"):

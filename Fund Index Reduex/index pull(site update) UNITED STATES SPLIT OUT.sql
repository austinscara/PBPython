	...
	when inf.SiteID is NULL
		then (case
				...
				when cc.id IN (106, 25, 148, 52, 192, 201, 112, 116, 119, 
					...
					then 'Africa'
				when cc.id IN (8, 81, 25, 139, 140, 40, 42, 145, 174, 180, 
										63, 195, 74, 76, 209, 210, 37, 132, 215)
					then 'Middle East'
				else 'Rest of World'
			end)
	else (case
				when cc2.id IN (2,3,4,6,10,12,13,15,17,21,22,23,24,
								26,27,29,32,33,36,38,43,44,45,46,47,
								49,51,53,56,58,59,60,61,62,66,67,69,
								70,71,75,108,130,134,160,162,186,230,232)
					then 'Europe'
				when cc2.id in (1)
					then 'US' -- US SPLIT OUT	
				when cc2.id IN (5,14,50, 134)
					then 'North America'
				when cc2.id IN (8, 11, 28, 102, 143, 196, 202, 205, 20, 31,
										72, 39, 172, 90, 161, 105, 110, 113, 34, 152, 
										167, 175, 191, 115, 99, 127, 35, 144, 48, 164, 
										57, 65, 73, 78, 20, 31, 72, 229)
					then 'Asia'
				when cc2.id IN (9, 55, 158, 98, 35, 168, 89, 189, 206, 159, 
										142, 154, 166, 173, 176, 179, 107, 125, 131, 
										171, 178, 183, 199, 200, 203, 208)
					then 'Oceania'
				when cc2.id IN (106, 25, 148, 52, 192, 201, 112, 116, 119, 
										141, 133, 136, 137, 147, 153, 155, 170, 103, 
										185, 188, 198, 117, 120, 121, 124, 182, 109, 
										118, 123, 126, 122, 129, 88, 150, 151, 156, 
										157, 163, 181, 182, 187, 190, 100, 204, 211, 
										212, 114, 146, 165, 68, 194, 239, 240, 241, 243)
					then 'Africa'
				when cc2.id IN (8, 81, 25, 139, 140, 40, 42, 145, 174, 180, 
										63, 195, 74, 76, 209, 210, 37, 132, 215)
					then 'Middle East'
				else 'Rest of World'
			end)
...
lEFT JOIN EntitySite es2 ON inf.SiteID = es2.EntityID
lEFT JOIN CountryCode cc2 ON es2.countryCode = cc2.ID



--- THIS IS WHERE YOU DECIDE WHICH FUNDS TO INCLUDE. CAN CUT IN PROGRAM, SO INCLUDE ALL YOU COULD WANT
WHERE inf.FundType in (
21,
24,
23,
1
